...
| `ERROR_TRACER_ADMIN_TOKEN` | 是 | — | 管理凭据，至少 24 个非空白可见 ASCII 字符 |
| `ERROR_TRACER_ADMIN_TOKEN_PREVIOUS` | 否 | 空 | 与当前令牌格式相同、在轮换期间临时接受的旧管理员令牌 |
...
管理员令牌通过 HTTP `Authorization` 请求头传输，因此服务端和 Dashboard
都会拒绝非 ASCII 字符、空白及控制字符。
...
指纹 v2 与已持久化的 v1 问题兼容。问题首次再次出现时，SQLite 会查找最后事件属于
同一 v2 分组的所有 v1 聚合记录，并在写入新事件前，以同一事务合并这些聚合及已保留
事件历史。即使带缓存参数的堆栈 URL 曾产生多个 v1 指纹，状态、累计次数和时间戳也会
保留。Schema v3 会持久标记 v1/v2 记录，并为有界的旧记录查找建立索引，正常采集
无需扫描项目的全部问题，已迁移分组也会被跳过；此外还会直接探测精确 v1 key，以
覆盖旧 NUL 分隔编码曾产生的碰撞聚合。尚未再次出现的 v1 问题会继续以原指纹显示，
直到需要执行这次惰性迁移。
